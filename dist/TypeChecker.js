...
class TypeChecker {
    ...
    infer(node, scope) {
        ...
        switch (node.tag) {
            ...
            //    break;
            case "Make":
                const type = scope.resolveNamedType(this.translateTypeNodeToType(node.type, scope));
                if (!(type instanceof StructType)) {
            ...
            case "TypeCheck":
                ...
                break;
            case "Cast":
                inferredType = this.translateTypeNodeToType(node.type, scope);
    ...
    inferData(node, scope) {
        return this.translateTypeNodeToType(new Parser_3.TypeDef(node.fieldDefs), scope);
    }
    ...
    inferSquareApply(node, scope) {
        ...
        if (calleeType.toString() == "Type") {
            const calleeAsType = scope.resolveNamedType(this.translateTypeNodeToType(node.callee, scope));
            if (calleeAsType instanceof SquareTypeToTypeLambdaType) {
                const actualParams = node.typeArgs.map((t) => this.translateTypeNodeToType(t, scope));
            ...
            const calledArgs = node.typeArgs.map((t) => this.translateTypeNodeToType(t, scope));
    ...
    inferRoundApply(node, scope) {
        ...
        if (node.callee instanceof Parser_3.Identifier &&
            ...
            const type = scope.resolveNamedType(this.translateTypeNodeToType(node.callee, scope));
            if (type instanceof StructType) {
                return type;
            }
    ...
    inferRoundValueToValueLambda(node, scope) {
        ...
        if (node.params[0] &&
            ...
            const paramType = new AppliedGenericType(innerScope.lookupType("Array"), [
                this.translateTypeNodeToType(node.params[0].type.expression, innerScope),
            ]);
        ...
        if (node.isTypeLambda &&
            ...
            if (node.explicitType) {
                const explicitType = this.translateTypeNodeToType(node.explicitType, innerScope);
    ...
    translateTypeNodeToType(node, scope) {
        ...
        switch (node.tag) {
            ...
            case "RoundValueToValueLambda":
                ...
                const type = new RoundValueToValueLambdaType(node.params.map((p) => this.translateTypeNodeToType(p, innerScope)), this.translateTypeNodeToType(node.block.statements[0], innerScope), node.isTypeLambda);
            ...
            case "SquareTypeToTypeLambda":
                ...
                return new SquareTypeToTypeLambdaType(genericParameters, this.translateTypeNodeToType(node.returnType, innerScope2));
            case "SquareApply":
                if (!(node instanceof Parser_1.SquareApply)) {
                    return new Type();
                }
                return new AppliedGenericType(this.translateTypeNodeToType(node.callee, scope), node.typeArgs.map((arg) => this.translateTypeNodeToType(arg, scope)));
            case "TypeDef":
    ...
    typeCheckApply(apply, scope) {
        ...
        if (apply.callee instanceof Parser_3.Identifier &&
            ...
            const type = scope.resolveNamedType(this.translateTypeNodeToType(apply.callee, scope));
            if (type instanceof StructType) {
    ...
    build(node, scope) {
        if (node instanceof Parser_3.Assignment) {
            ...
            if (node.condition instanceof Parser_1.TypeCheck &&
                ...
                const presumedType = this.translateTypeNodeToType(node.condition.type, trueScope);
    ...
    buildSymbolForAssignment(node, scope) {
        ...
        if (!node.value) {
            if (node.type && node.lhs instanceof Parser_3.Identifier) {
                const symbol = new Symbol(node.lhs.value, this.translateTypeNodeToType(node.type, scope), node);
        ...
        if (!node.type) {
            ...
            let nodeType = scope.resolveNamedType(this.translateTypeNodeToType(node.type, scope));
        ...
        if (node.lhs instanceof Parser_3.Identifier &&
            ...
            if (node.value instanceof Parser_3.RoundTypeToTypeLambda) {
                scope.declareType(node.lhs.value, this.translateTypeNodeToType(node.value, scope));
            }
            else {
                const symbolToDeclare = this.translateTypeNodeToType(node.value, scope);
