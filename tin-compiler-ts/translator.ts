...
export function translateFile(fileStatementsBlock) {
...
function createConstructor(typeDef) {
   const parameters =
      ...
      typeDef.fieldDefs.map((param, i) => {
         return `_p${i}${
            param.defaultValue ? " = " + translate(param.defaultValue) : ""
         }`;
      }) +
      ")";
...
function translate(term, args) {
   ...
   switch (term.tag) {
      ...
      case "Block":
         ...
         return [
            ...term.statements.map((st) => translate(st)),
      ...
      case "Assignment":
         return `${term.isDeclaration ? "var " : ""}${translate(term.lhs)} ${
            term.type
               ? `/*${translate(term.type)}*/`
               : term.typeSymbol
               ? `/* ${term.typeSymbol.toString()} */`
               : ""
      ...
      case "Cast":
         return `(${translate(term.expression)}) /* as ${translate(
            term.type
         )} */`;
      case "Identifier":
         return term.value;
      ...
      case "LambdaType":
         return `(${term.parameterTypes
            .map((t) => translate(t))
            .join(", ")}) => ${translate(term.returnType)}`;
      case "IfStatement":
         return `((${translate(term.condition)}) ? (${translate(
            term.trueBranch
         )}) : (${translate(term.falseBranch)})) `;
      case "BinaryExpression":
         if (term.operator === "=") {
            return `const ${term.left.value} ${
               term.left.type ? "/*" + term.left.type + "*/" : ""
            } = ${translate(term.right)}`;
         }
         if (term.operator === "|") {
            return translate({
               tag: "Apply",
               callee: { tag: "Identifier", value: "_TIN_UNION_OBJECTS" },
               args: [term.left, term.right],
            });
         }
         if (term.operator === "&") {
            return translate({
               tag: "Apply",
               callee: { tag: "Identifier", value: "_TIN_INTERSECT_OBJECTS" },
               args: [term.left, term.right],
            });
         }
         if (
      ...
      case "Apply":
         ...
         return (
            translate(term.callee) +
            open +
            term.args.map((arg) => translate(arg)).join(", ") +
            close
         );
      case "TypeDef":
         return `TIN_TYPE("${randomUUID()}", ${createConstructor(
            term
         )}, {${term.fieldDefs.map((f) => translate(f))}})`;
      case "FieldDef":
         return `${term.name}: { type: ${term.type}, defaultValue: ${translate(
            term.defaultValue
         )} }`;
      default:
         return "(? " + term.tag + " ?)";
